...
import { classesLisitngUrlSearchParamsSchema } from '../../utils/listingUrlSearchParams';
...
type FormFilterValues = FormQueryStateValues<ClassesListingSearchParams, 'status'>;
...
const UID = 'FORM_FILTER_LISTING_CLASSES';
export const ClassesFormSearchNFilter = ({
  formFilterValues,
  searchValue,
  onSearch,
  isSubmiting = false,
  ...
  const { handleSubmit, reset, watch, setValue } = useRemixForm<FormFilterValues>({
    ...
    resolver: zodResolver(classesLisitngUrlSearchParamsSchema),
    submitHandlers: {
      onValid: onFilter,
    },
  });
